...
static void on_realize(GtkGLArea *area) {
	...
	astroid.pos[0] = WIDTH / 4.0f;
	astroid.pos[1] = HEIGHT / 4.0f;
	astroid.pos[2] = 0.0f;
	astroid.rot = M_PI / 3.0f;
	astroid.dx = 0.8f;
	astroid.dy = 0.7f;
...
static void on_render(GtkGLArea *area, GdkGLContext *context) {
	...
	mat4_translate(astroid.pos, pos);
	mat4_rotate_z(astroid.rot, rot);
	...
	if(astroid.pos[0] > WIDTH - 40.0f) {
		astroid.pos[0] -= WIDTH;
		draw_mirror = TRUE;
	} else if(astroid.pos[0] < 40.0f) {
		astroid.pos[0] += WIDTH;
		draw_mirror = TRUE;
	}

	if(astroid.pos[1] > HEIGHT - 40.0f) {
		astroid.pos[1] -= HEIGHT;
		draw_mirror = TRUE;
	} else if(astroid.pos[1] < 40.0f) {
		astroid.pos[1] += HEIGHT;
		draw_mirror = TRUE;
	}
...
static gboolean on_idle(gpointer data) {
	...
	astroid.pos[0] += astroid.dx;
	astroid.pos[1] += astroid.dy;
